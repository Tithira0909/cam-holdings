...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        cb(null, 'co-' + Date.now() + path.extname(file.originalname));
    }
});
const upload = multer({ storage: storage });

const TABLE_NAME = 'construction_properties';

// --- ADMIN API ---

// GET All
router.get('/admin/construction-properties', authenticateToken, async (req, res) => {
    try {
...
// POST Create
router.post('/admin/construction-properties', authenticateToken, upload.fields([
...
// PUT Update
router.put('/admin/construction-properties/:id', authenticateToken, upload.fields([
    { name: 'main_image', maxCount: 1 },
    { name: 'sub_images', maxCount: 3 }
]), async (req, res) => {
    try {
...
// DELETE
router.delete('/admin/construction-properties/:id', authenticateToken, async (req, res) => {
    try {
