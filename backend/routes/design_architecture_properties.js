...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        cb(null, 'da-' + Date.now() + path.extname(file.originalname));
    }
});
const upload = multer({ storage: storage });

const TABLE_NAME = 'design_architecture_properties';

// --- ADMIN API ---

// GET All
router.get('/admin/design-architecture-properties', authenticateToken, async (req, res) => {
    try {
...
// POST Create
router.post('/admin/design-architecture-properties', authenticateToken, upload.fields([
...
// PUT Update
router.put('/admin/design-architecture-properties/:id', authenticateToken, upload.fields([
    { name: 'main_image', maxCount: 1 },
    { name: 'sub_images', maxCount: 3 }
]), async (req, res) => {
    try {
...
// DELETE
router.delete('/admin/design-architecture-properties/:id', authenticateToken, async (req, res) => {
    try {
...
// PATCH Toggle
router.patch('/admin/design-architecture-properties/:id/toggle', authenticateToken, async (req, res) => {
    try {
...
router.get('/public/design-architecture-properties/:id', async (req, res) => {
    try {
