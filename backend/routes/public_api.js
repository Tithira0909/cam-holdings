...
router.get('/services', async (req, res) => {
    try {
        ...
        let query = "SELECT s.*, st.name as service_type_name, st.slug as service_type_slug FROM services s LEFT JOIN service_types st ON s.service_type_id = st.id WHERE s.status = 'published'";
        const params = [];

        if (category) {
            query += " AND (st.slug = ? OR st.name = ?)";
