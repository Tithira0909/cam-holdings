...
async function setupDatabase() {
  ...
  try {
    ...
    const serviceMigrationQueries = [
        "ALTER TABLE services ADD COLUMN estimated_cost VARCHAR(255)"
    ];
    ...
    // Seed Service Types
    const serviceTypes = [
        { name: 'Real Estate & Development', slug: 'real_estate', description: 'Land acquisition, development, and sales.' },
        { name: 'Design & Architecture', slug: 'design_architecture', description: 'Architectural planning, 3D visualization, and design.' },
        { name: 'Construction & Management', slug: 'construction', description: 'Full build services and project management.' },
        { name: 'Interiors & Finishing', slug: 'interiors', description: 'Interior design, finishing, and custom joinery.' }
    ];

    for (const type of serviceTypes) {
        const [existing] = await db.query('SELECT * FROM service_types WHERE slug = ?', [type.slug]);
        if (existing.length === 0) {
            await db.query('INSERT INTO service_types (name, slug, description, status) VALUES (?, ?, ?, ?)',
                [type.name, type.slug, type.description, 'Active']);
            console.log(`Seeded Service Type: ${type.name}`);
        }
    }
