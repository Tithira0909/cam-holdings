...
def run():
    with sync_playwright() as p:
        ...
        if box:
            page.screenshot(path="verification/reviews_desktop.png", clip=box)
        else:
            page.screenshot(path="verification/reviews_desktop_full.png")
        ...
        section_m = page_mobile.locator("#reviews")
        section_m.scroll_into_view_if_needed()
        time.sleep(2)

        box_m = section_m.bounding_box()
        if box_m:
            page_mobile.screenshot(path="verification/reviews_mobile.png", clip=box_m)
