...
def run():
    # Start Vite server
    server_process = subprocess.Popen(["npx", "vite", "--port", "5173", "--host", "0.0.0.0"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    try:
        with sync_playwright() as p:
            ...
            # Force click step 2 tab (DESIGN)
            page.evaluate("document.querySelector('.itab[data-step=\"1\"]').click()")
            ...
            # Force click step 3 tab (BUILD)
            page.evaluate("document.querySelector('.itab[data-step=\"2\"]').click()")
            page.wait_for_timeout(1000)
            ...
            # Force click step 4 tab (FINISH)
            page.evaluate("document.querySelector('.itab[data-step=\"3\"]').click()")
            page.wait_for_timeout(1000)
