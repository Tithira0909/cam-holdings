...
const QuotationSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      const { data } = await api.get('/settings/quotation');
      setSettings(data);
    } catch (err) {
  ...
  const handleUpdate = async (key, value) => {
    try {
      await api.post('/settings/quotation', { key, value });
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Quotation Settings</h1>
